...
class AnchorsGenerator(nn.Module):
    ...
    def generate_anchors(self, scales, aspect_ratios, dtype=torch.float32, device=torch.device("cpu")):
        ...
        #  anchors （0, 0） , shape [len(ratios)*len(scales), 4]
        base_anchors = torch.stack([-ws, -hs, ws, hs], dim=1) / 2

        return base_anchors.round()  # round  
    ...
    def grid_anchors(self, grid_sizes, strides):
        ...
        #  grid_size，strides cell_anchors
...
def permute_and_flatten(layer, N, A, C, H, W):
    ...
    Returns:
        layer:  tensor ， reshape [N, -1, C]
    """
    # view reshape ， shape 
    # view tensor，permute tensor ， view 
    # reshape tensor 
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def __init__(self, anchor_generator, head,
        ...
        self.proposal_matcher = det_utils.Matcher(
            fg_iou_thresh,  #  iou fg_iou_thresh(0.7) 
            bg_iou_thresh,  #  iou bg_iou_thresh(0.3) 
    ...
    def assign_targets_to_anchors(self, anchors, targets):
        ...
        Returns:
            labels:  anchors （1, 0, -1 ， ， ）
                     ， RPN ， 1，0 
            matched_gt_boxes： anchors gt
        """
        labels = []
        matched_gt_boxes = []
        #  anchors targets
        for anchors_per_image, targets_per_image in zip(anchors, targets):
            ...
            if gt_boxes.numel() == 0:
                ...
                #  anchors gt iou （ iou<0.3 -1，0.3<iou<0.7 -2）
                matched_idxs = self.proposal_matcher(match_quality_matrix)
                # get the targets corresponding GT for each proposal
                # NB: need to clamp the indices because we can have a single
                # GT in the image, and matched_idxs can be -2, which goes
                # out of bounds
                #  clamp 0 anchors gt_boxes 
                #  ， 0
                #  labels_per_image ，
                #  gt_boxes ，
                #  。
    ...
    def _get_top_n_idx(self, objectness, num_anchors_per_level):
        ...
        """
         pre_nms_top_n anchors 
        Args:
            objectness: Tensor(  )
            num_anchors_per_level: List（ anchors ）
        Returns:
        ...
        for ob in objectness.split(num_anchors_per_level, 1):
            if torchvision._is_tracing():
                ...
                num_anchors = ob.shape[1]  #  anchors 
    ...
    def filter_proposals(self, proposals, objectness, image_shapes, num_anchors_per_level):
        ...
        """
         boxes ，nms ， post_nms_top_n 
        Args:
            proposals:  bbox 
            objectness:  
            image_shapes: batch size 
            num_anchors_per_level:  anchors 
    ...
    def compute_loss(self, objectness, pred_bbox_deltas, labels, regression_targets):
        ...
        Arguments:
            objectness (Tensor)： 
            pred_bbox_deltas (Tensor)： bbox regression
            labels (List[Tensor])：  1, 0, -1（batch labels List ）
            regression_targets (List[Tensor])： bbox regression
    ...
    def forward(self,
        ...
        #  bboxes regression 
        # objectness pred_bbox_deltas list
        objectness, pred_bbox_deltas = self.head(features)

        #  batch anchors ,list(tensor) batch_size
        ...
        if self.training:
            ...
            #  anchors gt， anchors ， ， anchors
            labels, matched_gt_boxes = self.assign_targets_to_anchors(anchors, targets)
            #  anchors gt， regression 
