...
def create_model(num_classes):
    ...
    # backbone = torch.nn.Sequential(*list(vgg_feature._modules.values())[:-1])  #   features  Maxpool  
    # backbone.out_channels = 512

    # https://download.pytorch.org/models/mobilenet_v2-b0353104.pth
    backbone = MobileNetV2(weights_path="./backbone/mobilenet_v2.pth").features
    backbone.out_channels = 1280  #   backbone  channels

    anchor_generator = AnchorsGenerator(sizes=((32, 64, 128, 256, 512),),
                                        aspect_ratios=((0.5, 1.0, 2.0),))

    roi_pooler = torchvision.ops.MultiScaleRoIAlign(featmap_names=['0'],  #   roi pooling
...
def main():
    ...
    #   collate_fn  ，  image  targets，  batch
    if train_sampler:
