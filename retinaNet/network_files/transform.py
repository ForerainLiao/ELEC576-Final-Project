...
def _resize_image(image, self_min_size, self_max_size):
    ...
    # interpolate   
    # image[None]   batch   [C, H, W] -> [1, C, H, W]
    # bilinear   4D Tensor
    image = torch.nn.functional.interpolate(
        image[None], scale_factor=scale_factor, mode="bilinear", recompute_scale_factor=True,
        align_corners=False)[0]
...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def resize(self, image, target):
        ...
        if self.training:
            size = float(self.torch_choice(self.min_size))  #    ,   self.min_size   min_size
        else:
            # FIXME assume for now that testing uses the largest scale
            size = float(self.min_size[-1])    #    ,   self.min_size   min_size
    ...
    def batch_images(self, images, size_divisible=32):
        ...
        #    shape   batch_shape   0   tensor
        batched_imgs = images[0].new_full(batch_shape, 0)
        for img, pad_img in zip(images, batched_imgs):
            #    images   batched_imgs   ，   ，   bboxes   
            #    batch   shape   
    ...
    def forward(self,
        ...
        for i in range(len(images)):
            ...
            image, target_index = self.resize(image, target_index)   #    bboxes   
            images[i] = image
        ...
        images = self.batch_images(images)  #    images   batch
